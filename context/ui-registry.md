...
- **Last updated:** 2026-09-08
- **Tokens/Classes:** static full-bleed hero — single image from `images.heroImage` (`src/assets/images.js` → `src/assets/hero_1.jpg`, the full 4032x3024 original, no srcSet/compression), `loading="eager"` + `fetchPriority="high"` (LCP, preload via `vite.config.js`), depth scrim `bg-gradient-to-t from-black/35 via-black/60 to-black/30`, headline `font-display text-5xl sm:text-6xl lg:text-7xl font-medium` (Manrope 500, lighter) with drop shadow, CTA `bg-primary text-primary-foreground rounded-sm px-7 py-3.5` (single "Explore Our Programs" link). Fonts: Manrope 400/500/600/700 + Plus Jakarta Sans variable, all self-hosted via fontsource in `main.jsx`.
